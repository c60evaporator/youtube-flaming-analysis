...
# 炎上前のデータ
df_influencer_before = df_influencer[
        (df_influencer['date'] > df_influencer['flaming_date'] - timedelta(days=BEFORE_FLAMING))
        & (df_influencer['date'] <= df_influencer['flaming_date'])]
# 炎上後のデータ
df_influencer_after = df_influencer[
        (df_influencer['date'] > df_influencer['flaming_date'])
        & (df_influencer['date'] <= df_influencer['flaming_date'] + timedelta(days=AFTER_FLAMING))]

# プロット用のメソッド
def plot_before_after(ax, df_before, df_after, x, y, flaming_date):
    ax.plot(df_before[x].values, df_before[y].values,
             c='blue', label='Before flaming')
    ax.plot(df_after[x].values, df_after[y].values,
            c='red', label='After flaming')
    ax.axvline(x=flaming_date, color='gray')
    ax.text(flaming_date, min(df_before[y].min(), df_after[y].min()),
            f'Flaming date = {flaming_date.strftime("%Y/%m/%d")}', verticalalignment='center', horizontalalignment='left')
    ax.set_title(y)
    ax.legend()

# 炎上前後の登録者数プロット
fig, axes = plt.subplots(2, 1, figsize=(10, 10))
plot_before_after(ax=axes[0], df_before=df_influencer_before, df_after=df_influencer_after,
                  x='date', y='subscriber_count', flaming_date=df_influencer['flaming_date'][0].to_pydatetime())

# 炎上前後の再生回数プロット
plot_before_after(ax=axes[1], df_before=df_influencer_before, df_after=df_influencer_after,
                  x='date', y='view_count', flaming_date=df_influencer['flaming_date'][0].to_pydatetime())

# %% 視聴回数のスペクトル解析
import numpy as np
# ハミング関数で両端を滑らかに
view_counts = df_influencer['view_count'].values
view_counts_hamming = (view_counts - np.mean(view_counts)) * np.hamming(len(view_counts))
# フーリエ変換
dft = np.fft.fft(view_counts_hamming)
freq = np.fft.fftfreq(len(view_counts), d=1.0)
# 変換結果をプロット
fig, axes = plt.subplots(4, 1, figsize=(10, 15))
axes[0].plot(range(len(view_counts)), view_counts)
axes[0].set_title('view_count')
axes[1].plot(range(len(view_counts)), view_counts_hamming)
axes[1].set_title('hamming')
axes[2].plot(1/freq[1:int(len(view_counts) / 2)], abs(dft[1:int(len(view_counts) / 2)]), c='orange')
axes[2].set_title('FFT')
axes[2].set_xscale('log')
axes[3].plot(1/freq[1:int(len(view_counts) / 2)], abs(dft[1:int(len(view_counts) / 2)]), c='darkorange')
axes[3].set_title('FFT 1-10 days')
axes[3].set_xlim([1, 10])

# %% 視聴回数を7日ごとにグルーピングして平均
# 炎上日からの経過日、経過週を計算
df_influencer['date_diff'] = (df_influencer['date'] - df_influencer['flaming_date']).dt.total_seconds() / 3600 / 24
df_influencer['week_diff'] = df_influencer['date_diff'].map(lambda x: math.floor((x-1)/7))
# 週ごとの合計視聴回数を求める
week_average = df_influencer.groupby('week_diff').agg({
    'date': 'min',
    'view_count': 'sum',
    'flaming_date': 'min'
})
# 炎上前のデータ
week_average_before = week_average[
        (week_average['date'] > week_average['flaming_date'] - timedelta(days=BEFORE_FLAMING))
        & (week_average['date'] <= week_average['flaming_date'])]
# 炎上後のデータ
week_average_after = week_average[
        (week_average['date'] > week_average['flaming_date'])
        & (week_average['date'] <= week_average['flaming_date'] + timedelta(days=AFTER_FLAMING))]
# 炎上前後の登録者数プロット
fig, axes = plt.subplots(1, 1, figsize=(10, 5))
# 炎上前後の再生回数プロット
plot_before_after(ax=axes, df_before=week_average_before, df_after=week_average_after,
                  x='date', y='view_count', flaming_date=week_average['flaming_date'][0].to_pydatetime())
